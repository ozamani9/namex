...
try:
    ...
    for r in q.all():
        ...
        try:
            current_app.logger.debug('nro_update - 1 - after try: {}'.format(r.nrNum))
            nr_service = NameRequestService()
            current_app.logger.debug('nro_update - 2 - after nr_service: {}')
            expiry_days = nr_service.get_expiry_days(r)
            current_app.logger.debug(f'nro_update - 3 - after expiry_days {expiry_days}')
            nro_data_pump_update(r, ora_cursor, expiry_days)
            current_app.logger.debug('nro_update - 6 - after nro_data_pump_call:')
            db.session.add(r)
            current_app.logger.debug('nro_update - 7 - after db save:')
